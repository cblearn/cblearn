...
class GNMDS(BaseEstimator, TripletEmbeddingMixin):
    """ Generaized Non-metric Multidimensional Scaling  (GNMDS).
    ...
    def __init__(self, n_components=2, max_iter=1000, verbose=False,
                 random_state: Union[None, int, np.random.RandomState] = None,
                 algorithm: str = "K", device: str = "auto"):
        ...
        Args:
            ...
            algorithm:
                The algorithm used to optimize the soft objective. {"K", "X"}
    ...
    def fit(self, X: utils.Questions, y: np.ndarray = None, init: np.ndarray = None,
        ...
        if self.algorithm == "X":
            ...
            result = torch_minimize_lbfgs(_gnmds_x_loss_torch, init, args=(triplets.astype(int)),
                                          device=self.device, max_iter=self.max_iter)
        elif self.algorithm == "K":
            result = minimize(_gnmds_k_loss, init.ravel(), args=(init.shape, triplets),
                              method='L-BFGS-B',
                              options=dict(maxiter=self.max_iter, disp=self.verbose))
        else:
            raise ValueError(f"Unknown GNMDS algorithm '{self.algorithm}'. Try 'K' or 'X'.")
...
def _gnmds_k_loss(x, x_shape, triplets, margin):
    """TODO: Loss using the kernel matrix """
    X = x.reshape(x_shape)
