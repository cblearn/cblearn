...
def query_accurary(true_response: utils.Answers, pred_response: utils.Answers) -> float:
...
def query_error(true_answers: utils.Answers, embedding_or_pred_answers: utils.Answers) -> float:
    return 1 - query_accurary(true_answers, embedding_or_pred_answers)


def _scorer(true_response, query):
    query, pred_response = utils.check_query_response(query, result_format='list-boolean')
    return query_accurary(true_response, pred_response)


TripletScorer = metrics.make_scorer(_scorer)
